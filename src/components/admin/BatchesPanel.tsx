  ...
  return (
    <section className={`rounded p-5 ${SURFACE}`}>
      <div className="mb-4 flex items-center gap-2">
        <FolderPlus className="h-4 w-4 text-[#8a6f66]" aria-hidden />
      ...
      <form action={action} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <label className="block">
          <span className="mb-1 block text-xs font-semibold text-[#9c8c86]">Medium (optional)</span>
...
function BatchList({
  ...
  return (
    <section className="rounded border border-[#2a2321] bg-[#1a1514]">
      <h2 className="border-b border-[#2a2321] px-4 py-3 text-sm font-bold">{title}</h2>
      {batches.length === 0 ? (
        <p className="px-4 py-4 text-sm text-[#6b5c57]">{empty}</p>
      ) : (
    ...
    <div className="space-y-6">
      <div>
        <Link
          ...
          className="inline-flex items-center gap-1.5 text-xs text-[#9c8c86] hover:text-[#c9b8b2]"
        >
      ...
      <section className="overflow-x-auto rounded border border-[#2a2321] bg-[#1a1514]">
        <h3 className="flex items-center gap-2 border-b border-[#2a2321] px-4 py-3 text-sm font-bold">
          <UsersRound className="h-4 w-4 text-[#8a6f66]" aria-hidden />
          Roster
        </h3>
        {members.length === 0 ? (
          <p className="px-4 py-4 text-sm text-[#6b5c57]">Nobody in this batch yet.</p>
        ) : (
            ...
            <tbody className="divide-y divide-[#2a2321]">
              {members.map((m) => (
                <tr key={m.uid}>
                  ...
                  <td className="px-3 py-2 text-xs text-[#9c8c86]">{m.school_name}</td>
                  {canEdit ? (
...
function TeachersOnBatch({
  ...
  return (
    <section className={`rounded p-5 ${SURFACE}`}>
      ...
      {assigned.length === 0 ? (
        <p className="text-sm text-[#6b5c57]">Nobody is assigned to this batch yet.</p>
      ) : (
        <ul className="space-y-2">
          {assigned.map((t) => (
            <li key={`${t.staff_id}-${t.subject ?? ""}`} className="flex items-center gap-3 text-sm">
              <span>{t.full_name}</span>
              <span className="font-mono text-xs text-[#6b5c57]">{t.staff_id}</span>
              {t.subject ? <span className="text-xs text-[#9c8c86]">{t.subject}</span> : null}
              {canEdit ? (
                <span className="ml-auto">
                  <RowAction
      ...
      {canEdit && free.length > 0 ? (
        <form action={action} className="mt-4 flex flex-wrap items-end gap-3">
          ...
          <label className="block">
            <span className="mb-1 block text-xs font-semibold text-[#9c8c86]">Assign a teacher</span>
    ...
    <section className={`rounded p-5 ${SURFACE}`}>
      ...
      <form action={action} className="space-y-3">
        ...
        <textarea
          ...
          className="w-full rounded border border-[#3a2f2c] bg-[#141010] px-3 py-2 font-mono text-sm
                     text-[#e8e0dc] outline-none placeholder:font-sans placeholder:text-[#6b5c57]
                     focus:border-[#8a6f66]"
        />
